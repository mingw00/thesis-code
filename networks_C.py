...
from partial_radon import PartialRadonStepEnd
...
class InvNet(torch.nn.Module, metaclass=ABCMeta):
    ...
    def _train_step(
        self, batch, loss_func_2, optimizer, acc_steps, theta, n_steps, signal_supported_inside_circle, geometric, index, pnet
    ):
        '''
        k = index
        ...
        pRadStep = PartialRadonStepEnd(theta=theta,steps=n_steps,circle=signal_supported_inside_circle, geometric=geometric)
        PY = pRadStep(x = inp_img, index = index)
        if index == 0:
            inp = PY[:,-1].unsqueeze(1)
        else:
            inp_i = PY[:,-1].unsqueeze(1)
            with torch.no_grad():
                inp = pnet(inp_i)
    ...
    def _val_step(self, batch, loss_func_2, theta, n_steps, signal_supported_inside_circle, geometric, index, pnet):
        inp_img, _ = batch
        inp_img = inp_img.to(self.device)
        ...
        for epoch in range(num_epochs):
            ...
            for i, batch in t:
                loss_b, inp, tar, pred = self._train_step(
                    batch, loss_func_2, optimizer, acc_steps, theta, n_steps, signal_supported_inside_circle, geometric, index, pnet
                )
                t.set_postfix(
            ...
            with torch.no_grad():
                ...
                for i, v_batch in enumerate(data_load_val):
                    v_loss_b, v_inp, v_tar, v_pred = self._val_step(
                        v_batch, loss_func_2, theta, n_steps, signal_supported_inside_circle, geometric, index, pnet
                    )
