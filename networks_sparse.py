...
from partial_radon import Radon
...
class InvNet(torch.nn.Module, metaclass=ABCMeta):
    ...
    def _train_step(
        self, batch, loss_func_2, optimizer, acc_steps, theta, theta_f, signal_supported_inside_circle
    ):
        '''
        pRas_s: sparse sinogram
        pRad: normal sinogram
        '''
        inp_img, _ = batch
        inp_img = inp_img.to(self.device)
        pRad = Radon(theta=theta, circle=signal_supported_inside_circle)
        pRad_s = Radon(theta=theta_f, circle=signal_supported_inside_circle)
    
        tar = pRad(inp_img)
        PY = pRad_s(inp_img)
        inp = F.interpolate(PY, size=[PY.size()[2], 180], mode='nearest')
    ...
    def _val_step(self, batch, loss_func_2, theta, theta_f, signal_supported_inside_circle):
        inp_img, _ = batch
        inp_img = inp_img.to(self.device)
        ...
        for epoch in range(num_epochs):
            ...
            for i, batch in t:
                loss_b, inp, tar, pred = self._train_step(
                    batch, loss_func_2, optimizer, acc_steps, theta, theta_f, signal_supported_inside_circle
                )
                t.set_postfix(
            ...
            with torch.no_grad():
                ...
                for i, v_batch in enumerate(data_load_val):
                    v_loss_b, v_inp, v_tar, v_pred = self._val_step(
                        v_batch, loss_func_2, theta, theta_f, signal_supported_inside_circle
                    )
